...
import com.pomona.batch.model.JobStatus
import com.pomona.batch.model.toView
...
class BatchAdminService(
    ...
    fun overview(): BatchOverview {
        ...
        return BatchOverview(
            ...
            jobs = listOf(WHOLESALE_JOB, RETAIL_JOB).map { JobStatus(it, lastDates[it], failures[it] ?: 0) },
        )
    }

    fun recentRuns(): List<BatchRunView> = batchRunRepository.findTop50ByOrderByIdDesc().map { it.toView() }

    fun failures(): List<BatchRunView> = batchRunRepository.findUnresolvedFailures().map { it.toView() }

    /** 같은 조건으로 한 건 다시. 하루치나 품목 하나라 금방 끝나므로 기다렸다가 새 실행 기록을 준다. */
    fun retry(id: Long): BatchRunView {
        val run = batchRunRepository.findByIdOrNull(id) ?: throw BatchRunNotFoundException(id)
        return rangeCollector.retry(run).toView()
    }
