...
class TradeItemConversionTest {
    ...
    private val hongro = Triple("06", "01", "17")
    private val varietyIds = mapOf(hongro to 1L)
    ...
    fun `자연키가 같은 행은 한 줄로 접히고 금액과 물량이 더해진다`() {
        val rows = listOf(
            trade(unitTotQty = "1925.000", totprc = "14488000.000"),
            trade(unitTotQty = "80.000", totprc = "144000.000"),
    ...
    fun `등급이나 산지가 다르면 따로 남는다`() {
        val rows = listOf(trade(grdCd = "11"), trade(grdCd = "12"), trade(plorCd = "568000")).toWholesaleRows(varietyIds)
    ...
    fun `물량이 0인 행은 집계에서 뺀다`() {
        ...
        val rows = listOf(trade(unitTotQty = "100.000"), trade(unitTotQty = "0.000", totprc = "0.000"))
            .toWholesaleRows(varietyIds)
    ...
    fun `물량 0인 행만 있는 묶음은 아예 나오지 않는다`() {
        assertThat(listOf(trade(unitTotQty = "0.000")).toWholesaleRows(varietyIds)).isEmpty()
    }
    ...
    fun `최저가와 최고가는 상자값을 kg 당으로 바꾼 뒤 고른다`() {
        ...
        val row = listOf(
            trade(unitQty = "10.000", lwprc = "11000.000", hgprc = "30000.000"),
            trade(unitQty = "11.500", lwprc = "22000.000", hgprc = "22000.000"),
        ).toWholesaleRows(varietyIds).single()
    ...
    fun `kg 환산 최저가는 내림 최고가는 올림한다`() {
        ...
        val row = listOf(trade(unitQty = "11.500", lwprc = "22000.000", hgprc = "22000.000"))
            .toWholesaleRows(varietyIds).single()
    ...
    @Test
    fun `산지명 꼬리 공백을 잘라낸다`() {
        val row = listOf(trade(plorNm = "충청북도 괴산군    ")).toWholesaleRows(varietyIds).single()
    ...
    fun `산지명이 null 이면 null 로 둔다`() {
        assertThat(listOf(trade(plorNm = null)).toWholesaleRows(varietyIds).single().plorNm).isNull()
    }

    @Test
    fun `필수값이 비어 있으면 예외를 던진다`() {
        // 실측에선 한 번도 없었다. 생기면 그 실행을 FAILED 로 남기고 원인을 본다.
        assertThatThrownBy { listOf(trade(totprc = null)).toWholesaleRows(varietyIds) }
    ...
    fun `품종은 대중소 코드 기준으로 한 번씩만 뽑는다`() {
        assertThat(listOf(trade(), trade(), trade(grdCd = "12")).toVarieties()).containsExactly(
    ...
    fun `품종명이 null 인 행과 있는 행이 섞이면 있는 쪽을 쓴다`() {
        assertThat(listOf(trade(sclsfNm = null), trade(sclsfNm = "홍로")).toVarieties().single().sclsfNm)
            .isEqualTo("홍로")
    }
}
