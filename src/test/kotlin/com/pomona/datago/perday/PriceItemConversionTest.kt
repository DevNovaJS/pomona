...
class PriceItemConversionTest {
    ...
    private fun survey(exmnDdPrc: String? = "25000", orgnlRegDt: String? = "2026-09-07T15:03:15Z") = PriceItem(
    ...
    fun `원본등록일시를 시각으로 바꾼다`() {
        assertThat(survey().toRow().orgnlRegDt)
            .isEqualTo(OffsetDateTime.of(2026, 9, 7, 15, 3, 15, 0, ZoneOffset.UTC))
    }

    @Test
    fun `원본등록일시가 없으면 null 로 둔다`() {
        assertThat(survey(orgnlRegDt = null).toRow().orgnlRegDt).isNull()
    }

    @Test
    fun `필수값이 비어 있으면 예외를 던진다`() {
        assertThatThrownBy { survey(exmnDdPrc = null).toRow() }
