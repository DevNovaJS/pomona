...
class PerDayPriceClientTest {
    ...
    private val perDayPriceClient = PerDayPriceClient(restClientBuilder.build(), dataGoUriFactory)
    ...
    fun `인코딩된 조건으로 호출하고 응답을 파싱한다`() {
        mockRestServiceServer.expect(requestTo(containsString("cond%5Bexmn_ymd%3A%3AGTE%5D=20260907")))
            .andRespond(withSuccess(fixture("perday-price-2rows.json"), MediaType.APPLICATION_JSON))

        val response = perDayPriceClient.fetchPage(request())
    ...
    fun `전체 건수가 한 페이지를 넘으면 다음 페이지까지 이어서 받는다`() {
        ...
        val items = perDayPriceClient.fetchAll(request().copy(numOfRows = 100))
    ...
    fun `결과 코드가 0이 아니면 예외를 던진다`() {
        mockRestServiceServer.expect(requestTo(containsString("perDay/price")))
            .andRespond(withSuccess(fixture("perday-price-error.json"), MediaType.APPLICATION_JSON))

        assertThatThrownBy { perDayPriceClient.fetchPage(request()) }
