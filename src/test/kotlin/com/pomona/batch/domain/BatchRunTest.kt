...
class BatchRunTest {
    ...
    private val date = LocalDate.of(2099, 1, 4)

    private fun newRun() = BatchRun(
        jobName = "wholesale-daily", targetDate = date,
        params = """{"whsl_mrkt_cd":"110001","gds_lclsf_cd":"06"}""",
    )

    @Test
    fun `시작하면 RUNNING 이고 종료시각이 없다`() {
        val run = batchRunRepository.save(newRun())
    ...
    fun `같은 날짜를 여러 번 재수집해도 이력이 전부 남는다`() {
        ...
        repeat(3) { batchRunRepository.save(newRun()).succeed(it) }
        batchRunRepository.flush()

        assertThat(batchRunRepository.findByJobNameAndTargetDateOrderByIdDesc("wholesale-daily", date)).hasSize(3)
    }

    @Test
    fun `jsonb 파라미터가 그대로 돌아온다`() {
        val saved = batchRunRepository.save(newRun())
        batchRunRepository.flush()

        assertThat(batchRunRepository.findById(saved.id!!).get().params).contains("110001")
    }
}
