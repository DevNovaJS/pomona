...
class BatchRunTest {
    ...
    fun `같은 날짜를 여러 번 재수집해도 이력이 전부 남는다`() {
        ...
        repeat(3) { runs.save(newRun()).succeed(it) }
        runs.flush()

        assertThat(runs.findByJobNameAndTargetDateOrderByIdDesc("wholesale-daily", date)).hasSize(3)
    }

    @Test
    fun `jsonb 파라미터가 그대로 돌아온다`() {
        val saved = runs.save(newRun())
        runs.flush()
